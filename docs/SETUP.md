...
| step | command | Verify |
|---|---|---|
| 1. Get the tool | `pipx install ai-costs` (or `pip install ai-costs`; the distribution is `ai-costs`, the command stays `ai-cost`). Or one executable file, no dependencies: download `ai-cost` from the release page of github.com/qmediat/ai-cost, `chmod +x ai-cost`, put it on `PATH` (for example `~/.local/bin/ai-cost`). From a checkout: `python3 scripts/build.py src dist/ai-cost` writes `dist/ai-cost` | `ai-cost --version` prints `ai-cost 2.4.0` (or newer) |
