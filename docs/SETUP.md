...
| step | command | Verify |
|---|---|---|
| 1. Get the tool | one executable file, no dependencies: download `ai-cost` from the release page of the repository you got this from, `chmod +x ai-cost`, put it on `PATH` (for example `~/.local/bin/ai-cost`). Once the package is on PyPI: `pipx install ai-costs` (or `pip install ai-costs`; the distribution is `ai-costs`, the command stays `ai-cost`). From a checkout: `python3 scripts/build.py src dist/ai-cost` writes `dist/ai-cost` | `ai-cost --version` prints `ai-cost 2.2.3` (or newer) |
