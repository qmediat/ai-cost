...
| github | Copilot: no price — the account's usage report (`providers.github.bill`); Actions per-minute for `--github` minutes without it | https://docs.github.com/en/rest/billing/usage · https://docs.github.com/en/billing/managing-billing-for-your-products/managing-billing-for-github-actions/about-billing-for-github-actions | Since 2026-06-01 Copilot is billed in AI credits (0.01 USD each) and the code review model is not disclosed: every review consumes its own number of credits, so a review has no price and `ai-cost` counts it. With `providers.github.bill` the amounts are the usage report's own per UTC day, SKU and repository — API-only = gross (before the credits a seat includes), real = net, seats as subscription shares — to the last digit in the report's "GitHub usage report" section. Actions: public repos free; private at the per-minute list price per runner OS, billable minutes from `/timing` only (a run's elapsed time is shown, never priced). |
